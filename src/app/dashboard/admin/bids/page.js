...
//         <div>
//             <div className="mb-6">
//                 <h1 className="text-2xl font-bold text-gray-800">View All Bids</h1>
//                 <p className="text-gray-500">Monitor all bidding activity across the platform</p>
//             </div>
...
//                                         Posted by: {task.userId?.name} • {task.category}
//                                     </p>
//                                 </div>
...
// import API from '@/utils/api';
// import toast from 'react-hot-toast';
// import { FiCheck, FiX, FiUser, FiClock, FiDollarSign } from 'react-icons/fi';

// export default function UserBidsPage() {
//     const [bids, setBids] = useState([]);
//     const [loading, setLoading] = useState(true);

//     // Backend-la irunthu User-oda all bids-a fetch pandrom
//     const fetchBids = async () => {
//         try {
//             const { data } = await API.get('/bids/user/all-bids');
//             setBids(data);
//         } catch (error) {
...
//     // Bid-a Accept pandrathukku
//     const handleAccept = async (bidId) => {
//         if (!window.confirm("Are you sure you want to accept this bid and assign the task?")) return;
        
//         try {
//             await API.put(`/bids/${bidId}/accept`);
//             toast.success('Bid Accepted! Task assigned successfully.');
...
//         <div>
//             <div className="mb-6">
//                 <h1 className="text-2xl font-bold text-gray-800">Received Bids</h1>
//                 <p className="text-gray-500">Review, accept, or reject bids placed on your tasks</p>
//             </div>

//             {loading ? (
//                 <div className="text-center py-20 text-gray-400 font-semibold text-lg animate-pulse">
//                     Loading bids...
//                 </div>
//             ) : bids.length === 0 ? (
...
//                     {bids.map((bid) => (
//                         <div key={bid._id} className="bg-white rounded-xl shadow-md p-6 border border-gray-100 hover:shadow-lg transition">
                            
//                             {/* Task & Status Header */}
//                             <div className="flex justify-between items-start mb-4 border-b pb-4">
//                                 <div>
//                                     <h3 className="font-bold text-lg text-blue-900 line-clamp-1">
//                                         Task: {bid.taskId?.title}
//                                     </h3>
//                                     <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
//                                         <FiUser /> {bid.taskerId?.name} 
//                                     </p>
//                                 </div>
//                                 <span className={`text-xs px-3 py-1 rounded-full font-bold uppercase tracking-wider ${
//                                     bid.bidStatus === 'accepted' ? 'bg-green-100 text-green-700' :
//                                     bid.bidStatus === 'rejected' ? 'bg-red-100 text-red-700' :
//                                     bid.bidStatus === 'withdrawn' ? 'bg-gray-100 text-gray-700' :
...
//                                         <FiDollarSign className="text-green-600"/> Bid Amount:
//                                     </span>
//                                     <span className="font-bold text-lg">₹{bid.bidAmount}</span>
//                                 </div>
//                                 <div className="flex justify-between text-gray-700">
//                                     <span className="flex items-center gap-2 font-medium">
//                                         <FiClock className="text-blue-600"/> Delivery Time:
//                                     </span>
//                                     <span className="font-bold">{bid.deliveryDays} days</span>
//                                 </div>
//                                 <div className="bg-gray-50 p-3 rounded-lg mt-3">
//                                     <p className="text-sm text-gray-600 italic">"{bid.proposalMessage}"</p>
//                                 </div>
//                             </div>

//                             {/* Actions (Only show if bid is pending) */}
...
//                                         className="flex-1 bg-red-50 text-red-600 py-2 rounded-lg font-semibold hover:bg-red-100 transition flex items-center justify-center gap-2"
//                                     >
//                                         <FiX /> Reject
//                                     </button>
//                                 </div>
...
//     const fetchBids = async () => {
//         try {
//             // 🟢 MATRAM INGE THAAN: Admin API-a call pandrom
//             const { data } = await API.get('/admin/bids');
//             setBids(data);
//         } catch (error) {
...
//         <div>
//             <div className="mb-6">
//                 <h1 className="text-2xl font-bold text-gray-800">Platform Bids</h1>
//                 <p className="text-gray-500">Monitor all bidding activity across the platform (Admin View)</p>
//             </div>
...
//             ) : bids.length === 0 ? (
//                 <div className="bg-white p-10 rounded-xl shadow-sm text-center border">
//                     <p className="text-gray-500 text-lg">No bids found on the platform yet.</p>
//                 </div>
//             ) : (
//                 <div className="grid gap-6 md:grid-cols-2">
//                     {bids.map((bid) => (
...
//                                 <div>
//                                     <h3 className="font-bold text-lg text-blue-900 line-clamp-1">
//                                         Task: {bid.taskId?.title || 'Unknown Task'}
//                                     </h3>
//                                     <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
//                                         <FiUser /> Tasker: {bid.taskerId?.name || 'Unknown User'} 
//                                     </p>
//                                 </div>
//                                 <span className={`text-xs px-3 py-1 rounded-full font-bold uppercase tracking-wider ${
//                                     bid.bidStatus === 'accepted' ? 'bg-green-100 text-green-700' :
...
//                             {/* Note: Admin can only VIEW bids, not accept/reject them. 
//                                 That is the User's job. So no buttons here. */}
//                         </div>
//                     ))}
//                 </div>
//             )}
                    ...
                    {bids.map((bid) => (
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-5 pb-5 border-b border-slate-100">
                                ...
                                <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border whitespace-nowrap ${
