...
//                         <p className="text-gray-500 text-sm">Tasker</p>
//                         <span className={`text-sm px-3 py-1 rounded-full font-medium mt-1 inline-block ${
...
export default function TaskerProfilePage() {
    ...
    const handleImageUpload = async (e) => {
        ...
        setUploading(true);
        try {
            // Note: Make sure your backend has a PUT route for updating user profile
            // Example: router.put('/profile', protect, upload.single('profileImage'), updateProfile)
            const { data } = await API.put('/auth/profile', formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            
            // Context-la irukka user data-va update pandrom
            updateUser({ profileImage: data.profileImage || data.user?.profileImage });
            toast.success('Profile photo updated!');
        } catch (error) {
            console.error(error);
            toast.error('Failed to upload photo. Check backend route.');
        } finally {
            setUploading(false);
        }
    };
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-10">
            ...
            <div>
                ...
                <p className="text-slate-500 text-lg">
                    Manage your identity, photo, and verification status.
                </p>
            </div>
                    ...
                    <div className="pt-20 sm:pt-24 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1.5 text-slate-500 font-medium">
                                <FiBriefcase className="text-blue-500" /> Professional Tasker
                            </div>
                        </div>
                    ...
                    <div className="bg-slate-50 rounded-[28px] p-3 sm:p-5 border border-slate-100">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <div className="flex items-center gap-4 p-4 sm:p-5 rounded-2xl bg-white border border-slate-100 shadow-[0_2px_10px_rgba(15,23,42,0.02)] hover:shadow-md transition-shadow">
                                ...
                                <div className="min-w-0">
                                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Account Status</p>
                                    <p className="font-semibold text-slate-900 capitalize truncate">{user?.verificationStatus}</p>
                                </div>
                            </div>
